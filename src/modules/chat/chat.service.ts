...
export class ChatService {
  ...
  async saveMessageGroup(content: string, currentUser: any, groupId: any) {
    ...
    await newMessage.save();
     for await (let users of integrants) {
          ...
          await this.notificationService.sendNewChatMessage(users.deviceToken);
        }
      }
    return newMessage;
  }
  ...
  async saveMessageAdmin(content: string, chatId: any, groupId: any, currentUser: any) {
    ...
    if (chat.adminUser == userId) {
       ...
       if (user.deviceToken) {
            await this.notificationService.sendNewChatMessage(chat.user.deviceToken);
          }
        } 
    if (chat.adminUser != userId) {
              const user = await this.usersService.findOneUser({_id: chat.adminUser, active: true })
       if (user.deviceToken) {
            await this.notificationService.sendNewChatMessage(chat.adminUser.deviceToken);
          }
        }
    return newMessage;
  }
